...
class ThresholdExtract:
    ...
    def setCrop(self, inputFolder, outputFolder):
        while not keyboard.is_pressed('d'):
            ...
            cv2.putText(Depth, 'Click upper, then lower bound y cordinate. Then press ENTER', (10, 700), cv2.FONT_HERSHEY_DUPLEX, 1, (255,255,255),2)
            cv2.imshow('Uncropped Image', Depth)
            cv2.setMouseCallback("Uncropped Image", self.on_click)
            cv2.waitKey()
            Depth = Depth[y1:y2, 0:Depth.shape[1]]
            cv2.putText(Depth, 'type d if done or r to set crop again', (80, 30), cv2.FONT_HERSHEY_DUPLEX, 1, (0,0,0),2)
            cv2.imshow('Uncropped Image', Depth)
            cv2.waitKey()
            #global y1, y2
        self.extract(y1, y2, inputFolder, outputFolder)



    def extract(self, y1Set, y2Set, inputFolder, outputFolder):
        ...
        for filename in os.listdir(inputFolder):
            if ("Depth" in filename) and filename.endswith('.png'):
                ...
                Depth = Depth[y1Set:y2Set, 0:Depth.shape[1]] #image crop
